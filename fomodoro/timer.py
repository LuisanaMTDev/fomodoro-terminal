...
from fomodoro.utils import States, INFO_FILE, BELL_SOUND_FILE, TIME_FORMAT
...
def run_timer(timer: Timer):
    ...
    while timer.state is States.START:
        ...
        struct_time_timer = gmtime(timer.break_time_in_seconds)
        ...
        try:
            ...
            if timer_character == "p":
                ...
                info["leftover_break_time_in_seconds"] = timer.break_time_in_seconds
                with open(INFO_FILE, 'w', encoding='utf-8') as info_file:
                    dump(info, info_file, indent=2)
        except curses.error:
